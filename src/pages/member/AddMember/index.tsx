...
export default function AddMember() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    API
      .post("/auth/signup", { name, username: email })
      .then((_) => {
  ...
  return (
    <div className="max-w-4xl mx-auto my-10 p-8 shadow rounded">
      <h1 className="text-4xl font-bold mb-4">Create Member</h1>
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div className="flex flex-col">
          ...
          <input className="name border p-2 rounded"
            ...
            required
          />
          <label className="text-2xl font-medium leading-none mb-2 mt-6" htmlFor="name">
            Email
          </label>
          <input className="name border p-2 rounded"
            type="email"
            ...
            required
          />
          <button className="update-button bg-gray-50 text-black hover:bg-indigo-500 border p-3 rounded text-1xl mt-4"
            type="submit">
            Register
          </button>
        </div>
