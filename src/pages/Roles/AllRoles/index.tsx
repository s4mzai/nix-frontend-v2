...
export default function AllRoles() {
  ...
  const handleDelete = (roleId) => {
    const choice = window.confirm("Are you sure you want to delete this role?");
    if (choice) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-12">
      ...
      <p className="text-lg text-center mt-4 mb-10">
        Everyone has a different role in Times. Hence different usage of the
        website.
      </p>
      {loading ? (
                ...
                <h4>
                  <ol className="flex flex-wrap gap-2 list-none">
                    {role.permissions.map((permissionId) => (
                      <li
                        className="text-lg bg-blue-500 rounded-full px-3 py-1"
                        key={permissionId}
                      >
